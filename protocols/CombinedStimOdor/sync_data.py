...
def sync_bpod_video(gpio_file_path, bpod_data_path):
    ...
    # 60:60+99 means only selecting the water delivery trials for lick detection
    ...
    for i, start_frame in enumerate(sync_pulse_start_frame):
        ...
        if tt == 0:
            ...
            s = paint(s, LED, 'LED', 'up')
            s = paint(s, OdorDelay, 'OdorDelay', 'down')
            s = paint(s, Odor, 'Odor', 'down')
            s = paint(s, RewardDelay[TrialTypes[i]-1], 'RewardDelay', 'down')
            s = paint(s, RewardValveTime, 'Reward', 'up')
