...
def detect_licks(
    ...
    save_full_video=False,         # if True, still save annotated video from frame 0 → end
    skip_head_seconds=0.0,         # detection disabled before this time
    skip_tail_seconds=0.0,         # detection disabled in the last N seconds
    save_only_trialtype=4,         # only save clips for this trial type; set None to disable filtering
):
    """
    ...
    # save_video_path is treated as an output directory for trial-type clips
    save_dir = None
    if save_video_path is not None:
        save_dir = Path(save_video_path)
    ...
    overlay_needed = show_video_with_licks or (save_dir is not None)
    ...
    while True:
        ...
        if detection_enabled and movement_detected and in_cooldown == 0:
            ...
            if current_frame_idx < len(lick_frames_array):
        ...
        try:
            ...
            this_trial_type = synced_gpio_file['trialtype'].iloc[current_frame_idx]
            this_trial_start = synced_gpio_file['trialstarttime'].iloc[current_frame_idx]
        except Exception:
            this_state = 'None'
            this_sync = 'None'
            this_trial_type = 'None'
            this_trial_start = None
        ...
        # Write edited frame only for selected trial type, one file per trial
        if save_dir is not None:
            frame_to_write = vis_frame if vis_frame is not None else frame

            try:
                is_target_trial = (
                    save_only_trialtype is None or
                    int(this_trial_type) == int(save_only_trialtype)
                )
            except Exception:
                is_target_trial = False

            valid_trial_start = (
                this_trial_start is not None and
                str(this_trial_start) != 'None' and
                not pd.isna(this_trial_start)
            )

            if is_target_trial and valid_trial_start:
                if current_saved_trial_id != this_trial_start:
                    if writer is not None:
                        writer.release()
                        writer = None

                    clip_name = f"trialtype_{int(this_trial_type)}_trialstart_{this_trial_start}.mp4"
                    clip_path = save_dir / clip_name
                    ...
                    if not writer.isOpened():
                        print(f"Warning: Could not open VideoWriter for {clip_path}.")
                        writer = None
                        current_saved_trial_id = None
                    else:
                        print(f"Saving clip: {clip_path}")
                        current_saved_trial_id = this_trial_start
...
def plot_lick_raster(
    ...
    mouse_id,
    date,
    fig_root_dir=Path(r"D:\lick_detection\out")
):
    """
...
if __name__ == "__main__":
    ...
    date = "2025" + input("Date (eg. 0126): 2025")

    # temp cache for long runs
    temp_data_folder = Path(r'D:\lick_detection\tmp')
    temp_data_folder.mkdir(parents=True, exist_ok=True)
    temp_data_file = temp_data_folder / f'CC20s' / f'{mouse_id}_{date}_temp.pkl'
    print(temp_data_file)
    lick_timestamps, video_duration, fps = None, None, None
    lick_frames_array, selected_roi, synced_gpio_file = None, None, None
    ...
    if temp_data_file.exists():
        ...
        root_video_dir = Path(r"\\140.247.90.110\homes2\Carol\VideoData20s")
        ...
        # Save only trial type 4 clips into this folder
        out_dir = Path(r"D:\lick_detection\out\trialtype4_clips")
        out_dir.mkdir(parents=True, exist_ok=True)

        # Run detection: start displaying from the same frame as ROI selection,
        # and save only trial type 4 clips.
        lick_timestamps, lick_frames_array, video_duration, fps = detect_licks(
            ...
            save_video_path=str(out_dir),   # [change here] now treated as output folder
            save_codec="mp4v",
            save_fps=30.0,
            ...
            skip_head_seconds=0,
            skip_tail_seconds=0,
            save_only_trialtype=4,          # [change here]
        )
    ...
    # save synced_gpio_file to tmp.csv
    synced_gpio_file.to_csv("tmp.csv", index=False)

    # Save global lick timestamps for MATLAB overlay
    lick_output_dir = Path(r"D:\lick_detection\out")
    lick_output_dir.mkdir(parents=True, exist_ok=True)

    lick_frame_idx = np.where(lick_frames_array == 1)[0]
    lick_df = synced_gpio_file.loc[lick_frame_idx, ['bpod_time_ms', 'trialtype']].copy()
    lick_df = lick_df.rename(columns={'bpod_time_ms': 'lick_time_ms'})
    lick_df = lick_df[lick_df['trialtype'] != 'None']
    lick_df.to_csv(lick_output_dir / f"{mouse_id}_{date}_lick_times_ms.csv", index=False)

    fig_path = plot_lick_raster(synced_gpio_file, fps, mouse_id, date)
    print(f"Saved raster: {fig_path}")
    print(f"Trial type 4 clips saved in: {out_dir}")